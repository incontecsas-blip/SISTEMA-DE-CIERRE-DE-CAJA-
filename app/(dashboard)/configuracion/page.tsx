...
export default function ConfiguracionPage() {
  ...
  const [ns, setNs] = useState<Partial<Servicio>>({ emoji: '📋', nombre: '', categoria: 'Otros', tipo: 'monto_variable', ganancia: 0.50, precioFijo: 0.50, gananciaMod: false });
  ...
  const saveCfg = async () => {
    try { await configAPI.save(cfg); notify('Guardado ✓'); }
  ...
  const addUser = async () => {
    ...
    try {
      ...
      notify((e as { response?: { data?: { error?: string } } })?.response?.data?.error ?? 'Error', 'error');
    }
  };
  ...
  const guardarPermisos = async () => {
    if (!uEdit) return;
    try {
      const { data } = await usuariosAPI.update(uEdit.id, { modulos: uEdit.modulos });
      setUsuarios(p => p.map(u => u.id === data.id ? data : u));
      setMPermisos(false); notify('Permisos actualizados');
    } catch { notify('Error', 'error'); }
  };
  ...
  const addSvc = async () => {
    ...
    setCfg(p => {
      const next = { ...p, servicios: [...p.servicios, newSvc] };
      configAPI.save(next).catch(() => notify('Error al guardar', 'error'));
      return next;
    });
    setMAddSvc(false);
    setNs({ emoji: '📋', nombre: '', categoria: 'Otros', tipo: 'monto_variable', ganancia: 0.50, precioFijo: 0.50, gananciaMod: false });
  ...
  const uploadLogo = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    r.onload = ev => setCfg(p => ({ ...p, logo: ev.target?.result as string }));
    r.readAsDataURL(f);
  };
  ...
  return (
    <div className="fade-in">
      ...
      {/* config-grid colapsa a 1 col en mobile */}
      <div className="config-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div className="card" style={{ padding: 17 }}>
          <p style={{ fontSize: 9, fontWeight: 800, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 12 }}>
          ...
          <div style={{ marginBottom: 12, padding: 12, background: 'var(--bg)', borderRadius: 10, border: '1.5px dashed var(--brd2)', textAlign: 'center' }}>
            {cfg.logo
              ? <img src={cfg.logo} style={{ maxHeight: 44, maxWidth: 170, objectFit: 'contain', display: 'block', margin: '0 auto 7px' }} alt="logo" />
              : <div style={{ color: 'var(--t4)', fontSize: 11.5, fontWeight: 700, marginBottom: 7 }}><i className="fas fa-image" style={{ fontSize: 18, display: 'block', marginBottom: 4 }}></i>Logo del negocio</div>
            }
              ...
              <div key={u.id} style={{ padding: '8px 11px', background: 'var(--bg)', borderRadius: 9, border: '1.5px solid var(--brd)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 800 }}>{u.nombre}</div>
                      <div style={{ fontSize: 9, color: 'var(--t3)', textTransform: 'uppercase', fontWeight: 700 }}>PIN: {u.pin} · {u.rol}</div>
                    </div>
                  </div>
                  {u.rol !== 'admin' && (
                    <div style={{ display: 'flex', gap: 5 }}>
                      <button onClick={() => { setUEdit({ ...u, modulos: [...(u.modulos ?? [])] }); setMPermisos(true); }} style={{ background: 'var(--cyl)', border: '1px solid var(--cy)', color: 'var(--cyd)', cursor: 'pointer', fontSize: 10, padding: '3px 8px', borderRadius: 6, fontFamily: 'Nunito,sans-serif', fontWeight: 700 }}>
                        <i className="fas fa-shield-halved" style={{ fontSize: 9, marginRight: 3 }}></i>Módulos
                      </button>
                      <button onClick={async () => { if (!confirm('¿Desactivar?')) return; await usuariosAPI.delete(u.id); setUsuarios(p => p.filter(x => x.id !== u.id)); }} style={{ background: 'none', border: 'none', color: 'var(--t4)', cursor: 'pointer', fontSize: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            ...
            {nu.rol === 'cajero' && (
              <div style={{ padding: 11, background: 'var(--bg)', borderRadius: 9, border: '1.5px solid var(--brd)' }}>
                <p style={{ fontSize: 9, fontWeight: 800, color: 'var(--t3)', textTransform: 'uppercase', marginBottom: 8 }}>
                  <i className="fas fa-shield-halved" style={{ color: 'var(--cy)', marginRight: 5 }}></i>Módulos habilitados
                </p>
      ...
      <div className="card" style={{ padding: 17, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <p style={{ fontSize: 9, fontWeight: 800, color: 'var(--t3)', textTransform: 'uppercase' }}>
            <i className="fas fa-th" style={{ marginRight: 6, color: 'var(--cy)' }}></i>Catálogo de Servicios
          </p>
      ...
      {mPermisos && uEdit && (
        <Modal onClose={() => setMPermisos(false)} width={400}>
          ...
          <p style={{ fontSize: 9, fontWeight: 800, color: 'var(--t3)', textTransform: 'uppercase', marginBottom: 11 }}>
            <i className="fas fa-shield-halved" style={{ color: 'var(--cy)', marginRight: 6 }}></i>Módulos accesibles
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 18 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div><label className="lbl">Categoría</label>
                <select value={ns.categoria ?? 'Otros'} onChange={e => setNs(p => ({ ...p, categoria: e.target.value }))} className="inp">
                  {['Recargas', 'Pagos', 'Trámites', 'Fotos', 'Copias', 'Otros'].map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div><label className="lbl">Tipo</label>
                <select value={ns.tipo ?? 'monto_variable'} onChange={e => setNs(p => ({ ...p, tipo: e.target.value as 'monto_variable' | 'cantidad' }))} className="inp">
            ...
            <div style={{ padding: '11px 13px', background: 'var(--mgl)', border: '1.5px solid var(--mg)', borderRadius: 10, display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }} onClick={() => setNs(p => ({ ...p, gananciaMod: !p.gananciaMod }))}>
              ...
              <div>
                ...
                <p style={{ fontSize: 10, color: 'var(--mgd)', marginTop: 2 }}>El cajero podrá modificar la ganancia en cada venta.</p>
              </div>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
      ...
      {mEditSvc && (
        <Modal onClose={() => setMEditSvc(false)} width={400}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 18 }}>
            ...
            <div><label className="lbl">Categoría</label>
              <select value={eS.categoria ?? 'Otros'} onChange={e => setES(p => ({ ...p, categoria: e.target.value }))} className="inp">
                {['Recargas', 'Pagos', 'Trámites', 'Fotos', 'Copias', 'Otros'].map(c => <option key={c}>{c}</option>)}
              </select>
            </div>
