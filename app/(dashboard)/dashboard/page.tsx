  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        <h1 style={{ fontSize: 20, fontWeight: 900 }}>Panel <span style={{ color: 'var(--cy)' }}>Principal</span></h1>
        <button className="btn btn-cy" onClick={() => router.push('/pos')}><i className="fas fa-plus"></i> Nueva Venta</button>
      </div>

      <DateBar f={f} setF={setF}><span style={{ fontSize: 11, color: 'var(--t3)', fontWeight: 600 }}>{vF.length} ventas</span></DateBar>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 14 }}>
        <StatCard label="Ventas"      icon="fas fa-receipt"    color="var(--cy)"  value={`$${D(totalV)}`}  sub={`${vF.length} transacciones`} />
        <StatCard label="Ganancia"    icon="fas fa-coins"      color="var(--grn)" value={`$${D(totalU)}`}  sub="Ganancia bruta" />
        <StatCard label="Egresos"     icon="fas fa-arrow-down" color="var(--red)" value={`$${D(totalE)}`}  sub={`${gF.length} registros`} />
        <div className="card" style={{ padding: 17, position: 'relative', overflow: 'hidden' }}>
          ...
          <p style={{ fontSize: 9, fontWeight: 800, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 5 }}>
            <i className="fas fa-chart-line" style={{ color: uNeta >= 0 ? 'var(--amb)' : 'var(--red)', marginRight: 4 }}></i>Utilidad Neta
          </p>
          <p className="mono" style={{ fontSize: 24, fontWeight: 700, color: uNeta >= 0 ? 'var(--amb)' : 'var(--red)' }}>${D(uNeta)}</p>
          <p style={{ fontSize: 10, color: 'var(--t4)', marginTop: 3 }}>Ganancia − Egresos</p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 12, marginBottom: 12 }}>
        <div className="card" style={{ padding: 17 }}>
          <p className="lbl" style={{ marginBottom: 10 }}>Ventas por Hora</p>
          <Bar data={{ labels: hrs.map(h => h + ':00'), datasets: [{ data: byHr, backgroundColor: 'rgba(0,193,212,.55)', borderColor: '#00C1D4', borderWidth: 1.5, borderRadius: 5 }] }} options={barOpts} style={{ maxHeight: 180 }} />
        </div>
        <div className="card" style={{ padding: 17 }}>
          <p className="lbl" style={{ marginBottom: 10 }}>Por Categoría</p>
          <Doughnut data={{ labels: Object.keys(cats).length ? Object.keys(cats) : ['Sin datos'], datasets: [{ data: Object.keys(cats).length ? Object.values(cats) : [1], backgroundColor: Object.keys(cats).length ? PAL : ['rgba(0,0,0,.06)'], borderWidth: 0 }] }} options={{ responsive: true, cutout: '60%', plugins: { legend: { position: 'bottom', labels: { color: '#7B93AE', font: { size: 9.5 as number }, padding: 8 } } } }} style={{ maxHeight: 180 }} />
        </div>
      </div>
      ...
      <div className="card" style={{ borderRadius: 14, overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead><tr><th>Hora</th><th>Cliente</th><th>Método</th><th>Cajero</th><th style={{ textAlign: 'right' }}>Total</th>{isAdmin && <th></th>}</tr></thead>
            <tbody>
              {vF.slice(0, 6).map(v => (
                <tr key={v.id}>
