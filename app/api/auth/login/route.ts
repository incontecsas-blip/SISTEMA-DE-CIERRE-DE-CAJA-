...
// GET — lista usuarios activos para el selector de login
export async function GET() {
  try {
    const rows = await query<{ id: number; nombre: string; rol: string }>(
      'SELECT id, nombre, rol FROM usuarios WHERE activo = true ORDER BY id'
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rows = await query<Usuario>(
      'SELECT * FROM usuarios WHERE pin = $1 AND activo = true', [pin]
    );
    if (!rows.length) return apiError('PIN incorrecto', 401);

    const user = rows[0];
    const token = await signToken({
      id: user.id, nombre: user.nombre, rol: user.rol, modulos: user.modulos,
    });
